...
#        data = np.log10(data)
      ...
      for i in range(0,len(ds[0])):
        ...
        sigma1 = empirical_hpd(good_sli,0.6827)
#        sigma1 = empirical_hpd(good_sli,0.3)
        percs2 = [sigma1[0],empirical_mode(good_sli),sigma1[1]]
...
def run_emcee(x,lnprob,args,nwalkers,nruns,fudge,chain_name,burns,pool=None,nthreads=1,namearray=[],headerinfo=[],sigmas=[],resume=False,w=False,shift_if_zero=1e-6):
...
def get_named_vals(chain_name,burns=0,names=[],use_hpd=False):
  ...
  for i in range(0,len(names)):
      ...
      try:
        ...
        sigma1 = empirical_hpd(d,0.6827)
  #        sigma1 = empirical_hpd(good_sli,0.3)
        percs2 = [sigma1[0],empirical_mode(d,frac_1sigma=frac_1sigma),sigma1[1]]
